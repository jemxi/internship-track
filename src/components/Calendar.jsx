...
const CELL = {
  ...
  holLogged: { bg:'#eff6ff', border:'#93c5fd', text:'#1d4ed8', sub:'#3b82f6' }, // holiday with hours = same as logged
}
...
export default function Calendar({
  ...
  onSave, // (date: Date, hours: number) => void  — passed from parent, same as addEntry
}) {
  const [pop, setPop]               = useState(null)
  const [holHours, setHolHours]     = useState(8)   // stepper value for holiday override
  ...
  const getStatus = (d) => {
    const e=entries[toStr(d)]
    if(!e) return null
    if(e.isAbsence) return 'absence'
    if(e.hours>0)   return 'logged'
    return null
  }
  const isInRange = (d) => {
    if(!startObj||!endObj) return false
    if(!workdays.includes(d.getDay())) return false
    if(holidays.includes(toStr(d))) return false
    return isWithinInterval(d,{start:startObj,end:endObj})
  }

  // Close popover on outside click
  useEffect(()=>{
    const fn=(e)=>{
      if(popRef.current&&!popRef.current.contains(e.target)&&calRef.current&&!calRef.current.contains(e.target))
        setPop(null)
    }
  ...
  const handleClick = (day, e) => {
    ...
    // Pre-fill stepper: if holiday already has hours, use those; else default to hoursPerDay
    setHolHours(existing?.hours > 0 ? Number(existing.hours) : Number(hoursPerDay))
    const r=e.currentTarget.getBoundingClientRect()
    const c=calRef.current?.getBoundingClientRect()
    setPop({ dateStr, top:r.top-(c?.top||0), left:r.left-(c?.left||0), dayW:r.width, dayH:r.height })
  }

  // Called when user clicks "Log Xh" or "Update" on a holiday
  const handleHolidayLog = (dateStr) => {
    if (!onSave) return
    onSave(parseLocalDate(dateStr), holHours)
    setPop(null)
  }

  // Called when user clicks "Remove hours" on a holiday
  const handleHolidayRemove = (dateStr) => {
    if (!onSave) return
    onSave(parseLocalDate(dateStr), 0)
    setPop(null)
  }
  ...
  const buildPop = () => {
    ...
    // Holiday that already has manually set hours
    const holLogged   = isHol && entry?.hours > 0 && !isAbsent
    ...
    // hoursDay: for holidays with override, show those hours; for pure holidays, 0
    const hoursDay    = isAbsent||(isHol&&!holLogged)||!workdays.includes(dow)
      ? 0
      : entry?.hours>0 ? Number(entry.hours) : Number(hoursPerDay)

    return { dayLabel:format(parseLocalDate(dateStr),'EEEE, MMMM d'), entry, isHol, holLogged, cumulative, isTodays, isFuture, remaining, pct, isAbsent, hoursDay }
  }
  ...
  const stepBtn = {
    ...
    background:'#fff', border:'1.5px solid #e2e4ee',
  ...
  return (
    <div ref={calRef} style={{ background:'#fff', borderRadius:16, border:'1px solid #eaecf4', boxShadow:'0 1px 6px rgba(80,80,140,.08)', padding:'22px 22px 18px', position:'relative', fontFamily:"'Inter',-apple-system,sans-serif" }}>
      <style>{`
        ...
        .hol-log-btn:hover{background:#7c3aed!important}
        .hol-upd-btn:hover{background:#1d4ed8!important}
        .hol-rem-btn:hover{background:#f0f0f8!important}
      `}</style>
        ...
        {weeks.map((week,wi)=>(
          <div key={wi} style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:4 }}>
            {week.map((day,di)=>{
              ...
              const isHol   = holidays.includes(toStr(day))
              const isTod   = isToday(day)
              const isSel   = isSameDay(day,selectedDate)
              const isPop   = pop?.dateStr===toStr(day)&&isCur
              const entry   = entries[toStr(day)]
              const hasHrs  = entry?.hours>0
              // Holiday that user manually logged hours on — show blue (same as logged)
              const holLogged = isHol && hasHrs && !entry?.isAbsence
              ...
              else if(holLogged||status==='logged') {bg=CELL.logged.bg;   border=`1.5px solid ${CELL.logged.border}`;   color=CELL.logged.text}
              ...
              return (
                <button key={di} className={isCur?'ccell':''} onClick={e=>isCur&&handleClick(day,e)}
                  ...
                  {isCur&&status==='absence'&&<span style={{fontSize:9,fontWeight:800,color:CELL.absence.sub,marginTop:2}}>ABSENT</span>}
                  {/* Holiday star — yellow if no hours logged, blue if hours logged */}
                  {isHol&&isCur&&<span style={{position:'absolute',top:2,right:3,fontSize:7,color:holLogged?'#3b82f6':'#f59e0b'}}>★</span>}
      ...
      {/* ── Popover ── */}
      {pop && pc && (()=>{
        const calW = calRef.current?.offsetWidth||500
        const popW = 290
        ...
        const accent = pc.isFuture?'#4f46e5':pc.isAbsent?'#e11d48':pc.holLogged?'#1d4ed8':'#6d51f7'
        ...
        // Header bg/border: holiday-logged = blue tint, plain holiday = yellow, etc.
        const hBg = pc.isAbsent?'#fff1f2':pc.holLogged?'#eff6ff':pc.isHol?'#fefce8':pc.isFuture?'#eef2ff':'#f0edff'
        const hBd = pc.isAbsent?'#fecdd3':pc.holLogged?'#bfdbfe':pc.isHol?'#fde68a':pc.isFuture?'#c7d2fe':'#ddd6fe'

        const headerLabel = pc.holLogged      ? '⭐ Holiday — hours logged'
          : pc.isHol                          ? '⭐ Philippine Holiday'
            ...
            <div style={{ background:'#fff', borderRadius:14, border:'1px solid #e5e7eb', boxShadow:'0 10px 36px rgba(30,31,46,.16)', overflow:'hidden' }}>
              ...
              <div style={{ background:hBg, borderBottom:`1px solid ${hBd}`, padding:'11px 15px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                <div>
                  ...
                  <div style={{ fontSize:11, color:'#9ca3af', marginTop:2 }}>{headerLabel}</div>
                </div>
              ...
              <div style={{ padding:'13px 15px', display:'flex', flexDirection:'column', gap:10 }}>
                ...
                {/* ══ HOLIDAY SECTION — only shown if onSave available ══ */}
                {pc.isHol && !pc.isAbsent && onSave && (
                  <div style={{ display:'flex', flexDirection:'column', gap:8 }}>

                    {pc.holLogged ? (
                      /* Already has hours — show current + option to update or remove */
                      <>
                        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                          <span style={{ fontSize:12, color:'#6b7280' }}>Hours logged this holiday</span>
                          <span style={{ fontSize:14, fontWeight:800, color:'#1d4ed8', background:'#eff6ff', padding:'2px 10px', borderRadius:7 }}>
                            {pc.entry.hours}h <span style={{ fontSize:10, color:'#9ca3af', fontWeight:400 }}>manual</span>
                          </span>
                        </div>
                        ...
                        <div style={{ display:'flex', alignItems:'center', gap:8, background:'#f7f8fc', borderRadius:11, padding:'6px 10px', border:'1.5px solid #e2e4ee' }}>
                          <button style={stepBtn} onClick={()=>setHolHours(h=>Math.max(1,h-1))}>−</button>
                          <span style={{ flex:1, textAlign:'center', fontSize:20, fontWeight:900, color:'#1a1a2e', letterSpacing:'-0.02em' }}>{holHours}h</span>
                          <button style={stepBtn} onClick={()=>setHolHours(h=>Math.min(24,h+1))}>+</button>
                        </div>

                        <div style={{ display:'flex', gap:8 }}>
                          <button className="hol-upd-btn" onClick={()=>handleHolidayLog(pop.dateStr)}
                            style={{ flex:1, padding:'8px 0', borderRadius:9, background:'#2563eb', color:'#fff', fontWeight:700, fontSize:12, border:'none', cursor:'pointer', transition:'background .15s' }}>
                            Update to {holHours}h
                          </button>
                          <button className="hol-rem-btn" onClick={()=>handleHolidayRemove(pop.dateStr)}
                            style={{ flex:1, padding:'8px 0', borderRadius:9, background:'#f7f8fc', color:'#6b7280', fontWeight:600, fontSize:12, border:'1.5px solid #e2e4ee', cursor:'pointer', transition:'background .15s' }}>
                            Remove hours
                          </button>
                        </div>
                      </>
                    ) : (
                      /* No hours yet — explain + offer to log */
                      <>
                        <div style={{ background:'#fefce8', border:'1px solid #fde68a', borderRadius:9, padding:'8px 12px', fontSize:11, color:'#92400e', fontWeight:500, lineHeight:1.5 }}>
                          ⭐ This is a PH holiday — not counted by default.<br/>
                          <strong>Did you work today?</strong> Log your hours below and they'll be added to your progress automatically.
                        </div>
                        ...
                        <button className="hol-log-btn" onClick={()=>handleHolidayLog(pop.dateStr)}
                          style={{ width:'100%', padding:'9px 0', borderRadius:9, background:'#6d51f7', color:'#fff', fontWeight:700, fontSize:12, border:'none', cursor:'pointer', transition:'background .15s' }}>
                          I worked this holiday — log {holHours}h
                        </button>
                      </>
                    )}
                ...
                {pc.cumulative > 0 && (<>
                  ...
                  <p style={{fontSize:12,color:'#374151',lineHeight:1.65,margin:0}}>
                    {pc.isFuture
                      ?(<>When you reach this date, you'll have <strong style={{color:'#4f46e5'}}>{pc.cumulative}/{targetHours} hrs</strong>.{' '}{pc.remaining>0?<span style={{color:'#9ca3af'}}>({pc.remaining} hrs to go)</span>:<strong style={{color:'#059669'}}>Goal achieved! 🎉</strong>}</>)
                      :pc.isTodays
                        ?(<>From the start you've reached <strong style={{color:'#6d51f7'}}>{pc.cumulative}/{targetHours} hrs</strong>.{' '}{pc.remaining>0?<><strong style={{color:'#ea580c'}}>{pc.remaining} hrs</strong> to go! 💪</>:<strong style={{color:'#059669'}}>Goal achieved! 🎉</strong>}</>)
                ...
                {pc.isHol && !pc.holLogged && !onSave && <p style={{fontSize:12,color:'#d97706',textAlign:'center',margin:0}}>Philippine holiday — not a work day.</p>}
